...
if graphics == javaGraphics: # use only with Jython
    from karel.javaworldadapter import RobotWorld 
    from karel.javaworldadapter import window   
    from karel.javaworldadapter import world
     
elif graphics == noGraphics:
    from karel.robotworld import RobotWorld
    from karel.robotworld import window
    from karel.robotworld import world
    
elif graphics == tkGraphics: # use only if Tkinter is installed
    from karel.tkworldadapter import RobotWorld
    from karel.tkworldadapter import world
    from karel.tkworldadapter import window
else  : # gtk graphics. Use only if pygtk is installed
    from karel.gtkworldadapter import RobotWorld
    from karel.gtkworldadapter import world
...
class UrRobot(_RobotSkeleton, Observable):
    ...
    def __init__(self, street, avenue, direction, beepers, fill = None, outline = 'black'):
...
#        directions = (north, west, south, east)
    ...
    def pickBeeper(self):
        ...
        try :
            ...
            self.setChanged()
#            self.notifyObservers(self.RobotState(self, self.pickBeeperAction))
            print ("No Beeper to pick at : " + str(data))
            raise NoBeepers(data)
#            world.error(NoBeepers() + ": " +data)
    ...
    def sleep(self):
        if world._RobotWorld__delay > 0 :
            time.sleep(world._RobotWorld__delay / 100.0) # FIXME: Tune this delay
